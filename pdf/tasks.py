...
def extract_text(doc):
    ...
    first_page = f_pdf[:-4]+'-1.html'

    # first_page = f_pdf[:f_pdf.rfind('/')+1]+'f8e72857-3a3c-493e-a361-6d111a272887-1.html'
    f = open(first_page, 'r')
    doc_text = f.read()
    f.close()

    doc.page_html = doc_text
    doc.save()
    return True
...
def process_file(doc):
    ...
    extract_text.delay(doc)
    os.system('pdftohtml -c '+f_pdf+' '+f_html)
